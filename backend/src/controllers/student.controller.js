...
export const fetchStudentSubjects = async (req,res) => {
    ...
    try {
        ...
        const findClassId = await Student.findById(findStudent.profileId).select('classId');
        if(!findClassId) return res.status(404).json({ success: false, message: "Class not found for the student" });

        
        const getSubjects = await Class.findById(findClassId.classId).select('subjectIds').populate({
            'path': 'subjectIds',
            'select': '-__v'
        });
        if(!getSubjects) return res.status(404).json({ success: false, message: "No subjects found for the student's class" });
        res.status(200).json({ success: true, subjects: getSubjects.subjectIds || [] });
...
export const fetchStudentAttendance = async (req,res) => {
    ...
    try {
        ...
        const attendanceRecords = await Attendance.find({
            ...
            month: month,
            subjectId: subjectId
        }).select('dialyAttendance -_id');

        console.log(attendanceRecords)
        if(!attendanceRecords) {
            return res.status(404).json({ success: false, message: "No attendance records found for the specified month and year" });
        }

        res.status(200).json({ success: true, attendance: attendanceRecords.length === 0 ? [] : attendanceRecords[0].dialyAttendance });
